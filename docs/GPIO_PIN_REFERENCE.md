...
|-----------|----------|----------|-----------|-------|
| **Yaw Servo** | GPIO 12 | PWM Signal | Output | Controls base rotation (consecutive pins) |
| **Pitch Servo** | GPIO 13 | PWM Signal | Output | Controls tilt forward/back (consecutive pins) |
| **Roll Servo** | GPIO 14 | PWM Signal | Output | Controls tilt left/right (consecutive pins) |
| **MPU6050 SDA** | GPIO 10 | I2C Data | Bidirectional | Gyro/Accelerometer data (consecutive pins) |
| **MPU6050 SCL** | GPIO 11 | I2C Clock | Output | Gyro/Accelerometer clock (consecutive pins) |
...
- **Physical Location**: Part of consecutive GPIO12-13-14 group for single header connection
- **Connection**:
  ```
...
- **All three servos use consecutive GPIO pins (12, 13, 14) for clean wiring**
- **Use a single 3-pin header for all servo signals**
...
#### 4. MPU6050 SDA - GPIO 10 (ESP32-S3)
...
- **Physical Location**: Part of consecutive GPIO10-11 group for single header connection
- **Connection**:
  ```
  ESP32-S3 GPIO 10 → MPU6050 SDA pin
  ```

#### 5. MPU6050 SCL - GPIO 11 (ESP32-S3)
...
- **Connection**:
  ```
  ESP32-S3 GPIO 11 → MPU6050 SCL pin
  ```
...
**Note**: GPIO 10 and 11 are physically consecutive on the ESP32-S3 board, allowing use of a single 4-pin header (3V3, GND, SDA, SCL) for clean wiring.
...
2. Test Servo (e.g., Yaw on GPIO 12):
   - ESP32 GPIO 12 → Servo Signal (Orange)
...
4. MPU6050 (for gyro-stabilization testing):
   - ESP32-S3 GPIO 10 → MPU6050 SDA (consecutive pins)
   - ESP32-S3 GPIO 11 → MPU6050 SCL (consecutive pins)
   - ESP32-S3 3.3V → MPU6050 VCC
   - ESP32-S3 GND → MPU6050 GND
   - Note: Use single 4-pin header for 3V3, GND, SDA, SCL
...
```
                        ...
                        ╚═══════════════╝
                         ...
                         │  │  │  │  │  └──── GPIO 12 → Yaw Servo Signal    ┐
                         │  │  │  │  └─────── GPIO 13 → Pitch Servo Signal  │ Consecutive
                         │  │  │  └────────── GPIO 14 → Roll Servo Signal   ┘ (Single Header)
                         │  │  └───────────── GPIO 10 → MPU6050 SDA  ┐ Consecutive
                         │  └──────────────── GPIO 11 → MPU6050 SCL  ┘ (Single Header)
    ...
    └──────────────┘        └────────────┘
         ...
         │  │                    │  │  │  └── GND → ESP32-S3 GND
         │  │                    │  │  └───── VCC → ESP32-S3 3.3V
         │  │                    │  └──────── SCL → GPIO 11 (consecutive)
         │  └────────────────────┴─────────── SDA → GPIO 10 (consecutive)
         │
         ├───────── Yaw Servo VCC (Red) - GPIO 12
         ├───────── Pitch Servo VCC (Red) - GPIO 13
         ├───────── Roll Servo VCC (Red) - GPIO 14
         │
...
```cpp
// Servo Pin Configuration (Consecutive pins for single header connection)
#define SERVO_PIN_YAW 12
#define SERVO_PIN_PITCH 13
#define SERVO_PIN_ROLL 14

// MPU6050 Configuration (I2C) - ESP32-S3 compatible consecutive pins
...
1. Check signal wire connection to correct GPIO (12, 13, or 14)
...
1. Verify I2C connections (GPIO 10 = SDA, GPIO 11 = SCL on ESP32-S3)
